...
@router.post("/scaling-policies", response_model=ScalingPolicySummary, status_code=status.HTTP_201_CREATED)
async def create_scaling_policy(
    ...
    task = await operation_task_service.create_task(
        operation_type="create_scaling_policy", target_type="scaling_policy", target_id=payload.name,
    )
    ...
    try:
        ...
        _ = await operation_task_service.update_task(task.id, state="succeeded", target_id=policy.id)
...
async def delete_scaling_policy(
    ...
    """오토스케일링 정책을 삭제합니다."""
    task = await operation_task_service.create_task(
        operation_type="delete_scaling_policy", target_type="scaling_policy", target_id=policy_id,
    )
    _ = await operation_task_service.update_task(task.id, state="running")
    try:
...
@router.post("/scheduled-tasks", response_model=ScheduledTaskSummary, status_code=status.HTTP_201_CREATED)
async def create_scheduled_task(
    ...
    task = await operation_task_service.create_task(
        operation_type="create_scheduled_task", target_type="scheduled_task", target_id=payload.name,
    )
    _ = await operation_task_service.update_task(task.id, state="running")
    try:
        st = await operations_service.create_scheduled_task(payload)
        _ = await operation_task_service.update_task(task.id, state="succeeded", target_id=st.id)
...
async def delete_scheduled_task(
    ...
    """예약 작업을 삭제합니다."""
    task = await operation_task_service.create_task(
        operation_type="delete_scheduled_task", target_type="scheduled_task", target_id=task_id,
    )
    _ = await operation_task_service.update_task(task.id, state="running")
    try:
