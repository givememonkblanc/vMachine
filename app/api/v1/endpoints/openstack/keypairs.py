...
from app.schemas.openstack.keypair import (
...
@router.post(
    "", response_model=KeypairCreateResponse, status_code=status.HTTP_201_CREATED
)
async def create_keypair(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(
            task.id, state="succeeded", target_id=payload.name
        )
...
async def delete_keypair(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(
            task.id, state="succeeded", target_id=keypair_name
        )
    except Exception as exc:
        _ = await operation_task_service.update_task(
            task.id, state="failed", target_id=keypair_name, error_message=str(exc)
        )
        raise
