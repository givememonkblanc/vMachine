...
def list_security_groups(
    security_group_service: Annotated[
        SecurityGroupService, Depends(get_security_group_service)
    ],
) -> SecurityGroupListResponse:
    """가상 네트워크 보안그룹 목록 조회"""
    return SecurityGroupListResponse(
        items=security_group_service.list_security_groups()
    )
...
@router.post(
    "", response_model=SecurityGroupCreateResponse, status_code=status.HTTP_201_CREATED
)
async def create_security_group(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(
            task.id, state="succeeded", target_id=response.security_group_id
        )
...
async def delete_security_group(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(
            task.id, state="succeeded", target_id=security_group_id
        )
    except Exception as exc:
        _ = await operation_task_service.update_task(
            task.id, state="failed", target_id=security_group_id, error_message=str(exc)
        )
        raise
...
@router.post(
    ...
    response_model=SecurityGroupRuleCreateResponse,
...
async def create_security_group_rule(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(
            task.id, state="succeeded", target_id=response.rule_id
        )
        return response.model_copy(update={"operation_task_id": task.id})
    except Exception as exc:
        _ = await operation_task_service.update_task(
...
async def delete_security_group_rule(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(
            task.id, state="succeeded", target_id=rule_id
        )
    except Exception as exc:
        _ = await operation_task_service.update_task(
            task.id, state="failed", target_id=rule_id, error_message=str(exc)
        )
        raise
