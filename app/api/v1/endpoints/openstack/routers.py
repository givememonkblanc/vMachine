...
@router.post("", response_model=RouterCreateResponse, status_code=status.HTTP_201_CREATED)
async def create_router(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(task.id, state="succeeded", target_id=response.router_id)
...
async def delete_router(
    ...
    try:
        ...
        _ = await operation_task_service.update_task(task.id, state="succeeded", target_id=router_id)
    except Exception as exc:
        _ = await operation_task_service.update_task(task.id, state="failed", target_id=router_id, error_message=str(exc))
        raise
...
@router.delete("/{router_id}/interfaces/{subnet_id}", status_code=status.HTTP_204_NO_CONTENT)
