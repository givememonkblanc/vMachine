...
_vmware_pool: VMwareConnectionPool | None = None
...
def get_vmware_pool() -> VMwareConnectionPool:
    global _vmware_pool
    if _vmware_pool is None:
        _vmware_pool = VMwareConnectionPool(get_settings())
    return _vmware_pool
...
def get_vmware_factory() -> VMwareClientFactory:
    ...
    if _vmware_factory is None:
        _vmware_factory = VMwareClientFactory(get_settings(), pool=get_vmware_pool())
...
def get_parallel_assessment_service() -> ParallelAssessmentService:
    return ParallelAssessmentService(
        inventory_service=get_vmware_inventory_service(),
        compatibility_service=get_vmware_compatibility_service(),
        mapping_engine=get_vmware_mapping_engine(),
        operation_task_service=get_operation_task_service(),
    )
