...
class VMwareCompatibilityService:
    ...
    def evaluate(self, vm: VMSummary) -> ScoredCompatibilityResult:
        ...
        if compatible:
            ...
            summary_text = (
                f"Incompatible — {critical_count} critical issues (score: {score:.2f})"
            )
    ...
    @staticmethod
    def _check_power_state(
        vm: VMSummary, issues: list[CompatibilityIssueDetail]
    ) -> None:
