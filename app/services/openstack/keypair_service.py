...
class KeypairService:
    ...
    def get_keypair(self, keypair_name: str) -> KeypairSummary:
        kp = self.factory.call(
            "compute", "find_keypair", keypair_name, ignore_missing=True
        )
        if not kp:
            raise AppException(
    ...
    def create_keypair(self, payload: KeypairCreateRequest) -> KeypairSummary:
        existing = self.factory.call(
            "compute", "find_keypair", payload.name, ignore_missing=True
        )
        if existing:
            raise AppException(
    ...
    def delete_keypair(self, keypair_name: str) -> None:
        deleted = self.factory.call(
            "compute", "delete_keypair", keypair_name, ignore_missing=True
        )
        if deleted is False:
            raise AppException(
                message="Keypair not found",
