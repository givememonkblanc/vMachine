...
class FlavorMatchResult(BaseModel):
    ...
    overprovisioned: bool = Field(
        default=False, description="VM 사양이 Flavor보다 낮음 (리소스 낭비)"
    )
    underprovisioned: bool = Field(
        default=False, description="VM 사양이 Flavor보다 높음 (성능 부족 위험)"
    )


class NetworkMappingResult(BaseModel):
    vm_network: str = Field(description="VMware 네트워크/포트 그룹 이름")
    openstack_network_id: str | None = Field(
        default=None, description="매칭된 OpenStack 네트워크 UUID"
    )
    openstack_network_name: str | None = Field(
        default=None, description="매칭된 OpenStack 네트워크 이름"
    )
    match_type: str = Field(
        description="매칭 방식 (exact_name, case_insensitive, vlan_id, cidr, not_found)"
    )
...
class DiskMappingResult(BaseModel):
    ...
    openstack_volume_type: str | None = Field(
        default=None, description="추천 OpenStack 볼륨 타입"
    )
...
class VMCompatibilityResult(BaseModel):
    ...
    issues: list[str] = Field(
        default_factory=list, description="발견된 호환성 문제 목록"
    )
...
class VMMappingResult(BaseModel):
    vm_id: str = Field(description="VMware VM MOR")
    vm_name: str = Field(description="VM 이름")
    flavor_match: FlavorMatchResult | None = Field(
        default=None, description="매칭된 Flavor"
    )
    network_mappings: list[NetworkMappingResult] = Field(
        default_factory=list, description="네트워크 매핑 결과"
    )
    disk_mappings: list[DiskMappingResult] = Field(
        default_factory=list, description="디스크 매핑 결과"
    )


class AssessmentRequest(BaseModel):
    vm_ids: list[str] = Field(
        ..., min_length=1, description="평가할 VMware VM MOR 목록"
    )
    include_mapping: bool = Field(
        default=True, description="OpenStack 리소스 매핑 포함 여부"
    )
...
class AssessmentResult(BaseModel):
    vm_id: str = Field(description="VMware VM MOR")
    vm_name: str = Field(description="VM 이름")
    compatibility: "ScoredCompatibilityResult" = Field(
        description="호환성 평가 결과 (scored)"
    )
    mapping: VMMappingResult | None = Field(
        default=None, description="리소스 매핑 결과"
    )
...
class AssessmentResponse(BaseModel):
    ...
    summary: dict[str, int] = Field(
        description="요약 통계 (total, compatible, incompatible, warning_count)"
    )
...
class MigrationStep(BaseModel):
    ...
    action: str = Field(
        description="액션 유형 (create_flavor, create_network, create_volume, import_image, create_server, cleanup)"
    )
...
class MigrationPlanVM(BaseModel):
    ...
    vm_name: str = Field(description="VM 이름")
    priority: int = Field(
        default=5, ge=1, le=10, description="마이그레이션 우선순위 (1=가장 높음)"
    )
    target_flavor_id: str | None = Field(
        default=None, description="대상 OpenStack Flavor ID"
    )
    target_network_ids: list[str] = Field(
        default_factory=list, description="대상 OpenStack 네트워크 UUID 목록"
    )
    target_volume_types: list[str] = Field(
        default_factory=list, description="대상 볼륨 타입 목록"
    )
    estimated_downtime_minutes: int = Field(default=0, description="예상 다운타임 (분)")
    steps: list[MigrationStep] = Field(
        default_factory=list, description="실행 단계 목록"
    )
    estimated_total_minutes: int = Field(
        default=0, description="전체 예상 소요 시간 (분)"
    )


class MigrationPlanRequest(BaseModel):
    vm_ids: list[str] = Field(
        ..., min_length=1, description="마이그레이션 계획을 수립할 VM 목록"
    )
    priority_overrides: dict[str, int] = Field(
        default_factory=dict, description="VM별 우선순위 오버라이드"
    )
...
class CompatibilityIssueDetail(BaseModel):
    ...
    category: str = Field(
        description="문제 카테고리 (os, cpu, memory, disk, network, disk_controller, firmware, secure_boot, vmware_tools, nic)"
    )
    message: str = Field(description="문제 설명")
    compatible: bool = Field(
        default=False, description="이 문제가 호환성에 영향을 미치는지 여부"
    )
...
class ScoredCompatibilityResult(BaseModel):
    ...
    issues: list[CompatibilityIssueDetail] = Field(
        default_factory=list, description="발견된 문제 상세 목록"
    )
...
class ParallelAssessmentRequest(BaseModel):
    vm_ids: list[str] = Field(
        ...
        min_length=1,
        max_length=500,
        description="평가할 VMware VM MOR 목록 (최대 500개)",
    )
    include_mapping: bool = Field(
        default=True, description="OpenStack 리소스 매핑 포함 여부"
    )
    max_concurrency: int = Field(
        default=10, ge=1, le=50, description="최대 동시 평가 수"
    )
    timeout_seconds: int = Field(
        default=300, ge=10, le=3600, description="평가 타임아웃 (초)"
    )
...
class ParallelAssessmentProgress(BaseModel):
    ...
    status: str = Field(
        default="running", description="작업 상태 (running, completed, failed, timeout)"
    )
