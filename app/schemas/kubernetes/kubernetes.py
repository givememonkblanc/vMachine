...
class PodSummary(BaseModel):
    ...
    status: str = Field(
        description="Pod 상태 (Running, Pending, Succeeded, Failed, Unknown)"
    )
    ...
    containers: list[ContainerSummary] = Field(
        default_factory=list, description="컨테이너 목록"
    )
...
class PodCreateRequest(BaseModel):
    ...
    port: int | None = Field(
        default=None, ge=1, le=65535, description="컨테이너 노출 포트"
    )
    labels: dict[str, str] = Field(default_factory=dict, description="레이블")
...
class ServiceCreateRequest(BaseModel):
    ...
    target_port: int | None = Field(
        default=None, ge=1, le=65535, description="대상 컨테이너 포트 (기본값: port)"
    )
    selector: dict[str, str] = Field(
        default_factory=dict, description="Pod 선택자 레이블"
    )
