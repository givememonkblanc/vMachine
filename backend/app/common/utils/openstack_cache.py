...
_cache_tracer = get_tracer("cache")
...
def cache_get(resource: str) -> list[Any] | None:
    ...
    with _cache_tracer.start_as_current_span(
        "cache.get",
        attributes={"cache.resource": resource, "cache.backend": backend},
    ) as span:
        if _backend is not None:
            result = _backend.get(resource)
...
def cache_set(resource: str, value: list[Any]) -> None:
    ...
    with _cache_tracer.start_as_current_span(
        ...
        if _backend is not None:
            ...
            if span.is_recording():
                span.set_attribute("cache.value_count", len(value) if value else 0)
