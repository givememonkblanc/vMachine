...
class FlavorService:
    ...
    def list_flavors(self) -> list[FlavorSummary]:
        return [
            FlavorSummary(**{"id": flavor.id, "name": flavor.name, "vcpus": flavor.vcpus, "ram": flavor.ram, "disk": flavor.disk})
            for flavor in self.factory.call("compute", "flavors", limit=self._list_limit)
        ]
    ...
    def get_flavor(self, flavor_id: str) -> FlavorSummary:
        ...
        return FlavorSummary(
            id=flavor.id, name=flavor.name, vcpus=flavor.vcpus, ram=flavor.ram, disk=flavor.disk
        )
    ...
    def delete_flavor(self, flavor_id: str) -> None:
        deleted = self.factory.call("compute", "delete_flavor", flavor_id, ignore_missing=True)
        if deleted is False:
            raise AppException(message="Flavor not found", status_code=404, error_code="flavor_not_found")
