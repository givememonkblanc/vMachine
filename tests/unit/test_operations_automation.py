...
def test_scaling_policy_crud(client: TestClient) -> None:
    mock_svc = MagicMock()
    mock_svc.list_scaling_policies = AsyncMock(
        return_value=ScalingPolicyListResponse(items=[])
    )
    mock_svc.get_scaling_policy = AsyncMock(
        return_value=ScalingPolicySummary(
            id="sp-1",
            name="cpu-scale",
    ...
    try:
        ...
        res = client.post(
            "/api/v1/operations/scaling-policies",
            json={
                "name": "cpu-scale",
                "metric_name": "cpu_usage",
                "threshold": 80.0,
...
def test_scheduled_task_crud(client: TestClient) -> None:
    mock_svc = MagicMock()
    mock_svc.list_scheduled_tasks = AsyncMock(
        return_value=ScheduledTaskListResponse(items=[])
    )
    mock_svc.get_scheduled_task = AsyncMock(
        return_value=ScheduledTaskSummary(
            id="st-1",
            name="daily-backup",
            task_type="backup",
            cron_expression="0 3 * * *",
            target_action="create_snapshot",
            enabled=True,
        )
    )
    ...
    try:
        ...
        res = client.post(
            "/api/v1/operations/scheduled-tasks",
            json={
                "name": "daily-backup",
                "task_type": "backup",
                "cron_expression": "0 3 * * *",
...
def test_storage_pool_crud(client: TestClient) -> None:
    ...
    mock_svc.get_pool = AsyncMock(
        return_value=StoragePoolSummary(
            id="pool-1",
            name="ceph-primary",
            pool_type="ceph",
            status="active",
            total_capacity_gb=10240,
            used_capacity_gb=3072,
