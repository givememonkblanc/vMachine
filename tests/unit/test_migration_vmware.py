...
async def test_execute_vmware_migration():
    ...
    conn_mock.compute.find_flavor.return_value = flavor_mock
    
    network_mock = MagicMock(id="net-123")
    conn_mock.network.find_network.return_value = network_mock
    ...
    with patch("app.modules.migration.manager.SessionLocal", return_value=mock_session):
    ...
    # Verify OpenStack calls
    conn_mock.image.create_image.assert_called_once_with(
        name="migrated-test-vm",
        filename="/tmp/migrations/test-vm.vmdk",
        disk_format="vmdk",
